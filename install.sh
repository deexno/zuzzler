...
ensure_venv() {
  ...
  if ! "$VENV_DIR/bin/python" -m pip --version >/dev/null 2>&1; then
    fail "Virtual environment was created, but pip is still unavailable. On Debian/Ubuntu verify python3-venv and python3-full, then remove $VENV_DIR and retry."
  fi
}
